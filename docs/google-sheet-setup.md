...
Set this environment variable where the app runs (e.g. Replit, Railway, or local `.env`):

```bash
GOOGLE_SHEET_CSV_URL=https://docs.google.com/spreadsheets/d/e/2PACX-.../pub?output=csv&gid=914273778
```

Use the **exact** URL you got from “Publish to web”.
...
- **503 Form sheet not configured** — `GOOGLE_SHEET_CSV_URL` is not set on the server.
