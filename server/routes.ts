...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Webhook handler to process payments and create registrations
        ...
        if (session.payment_status === 'paid') {
          try {
            ...
            writeRosterRecord({
              ...
              week_id: items[0]?.week_id || 'unknown',
              week_label: items[0]?.week_label || 'Unknown Week',
              student_name: items[0]?.student_name || childName,
            ...
            console.log(`Stored roster records for session ${session.id}`);
          } catch (error) {
