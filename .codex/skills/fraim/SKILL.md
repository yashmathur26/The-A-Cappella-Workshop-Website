...
0. **Preload deferred FRAIM tools when needed**:
   - If FRAIM MCP tools are unavailable because this host lazily loads deferred tool schemas, call `tool_search` once to load `fraim_connect`, `list_fraim_jobs`, `get_fraim_job`, `get_fraim_file`, `seekMentoring`.
